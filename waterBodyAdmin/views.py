...
class CardSummaryView(APIView):
    def get(self,request):
     ...
     numberofWaterBodySurveyResponse = WaterBodySurveyResponse.objects.aggregate(NumberofWaterBodyResponse=Count("uniqueid"))
...
class AccessRightsViewSet(ModelViewSet):
    ...
    def get_serializer_class(self):
         if self.request.method == 'POST':
             return AccessRightsCreateSerializer
         elif self.request.method == 'PATCH':
             return AccessRightsCreateSerializer
         return AccessRightsListSerializer

    @action(methods=['GET'], detail=False)
    def getRolePermissions(self, request, *args, **kwargs):
        role_id = str(kwargs['role_id'])
        logger.info(role_id)
        query = '''SELECT 
	       "waterBodyAdmin_resourcepermission".permission_id,
	       "waterBodyAdmin_resourcepermission".resource_id,
	       "waterBodyAdmin_accessrights".id AS accessright_id,
           	"waterBodyAdmin_resource".name AS resourcename,
	        "waterBodyAdmin_permissiontype".name AS permissionname,
            "waterBodyAdmin_resourcepermission".id,
            "waterBodyAdmin_accessrights".id as accessright_id
FROM
...
class WaterBodyTankTypeViewSet(ModelViewSet):
    ...
    serializer_class = WaterBodyTankTypeSerializer
    filter_backends = (SearchFilter, OrderingFilter)
    pagination_class = LimitOffsetPagination
    search_fields = [ 'name' ]
    ordering_fields = [ 'name' ]
    ...
    @action(detail=False, methods=['GET'])
    def All(self,request):
        queryset = WaterBodyTankType.objects.values('id','name')
        if request.method == 'GET':
            return Response(list(queryset))

class WaterBodyAvailabilityViewSet(ModelViewSet):
    http_method_names = ['get','post','patch','delete']
    queryset = WaterBodyAvailability.objects.all()
    serializer_class = WaterBodyAvailabilitySerializer
    filter_backends = (SearchFilter, OrderingFilter)
    pagination_class = LimitOffsetPagination
    search_fields = [ 'name' ]
    ordering_fields = [ 'name' ]
    ...
    @action(detail=False, methods=['GET'])
    def All(self,request):
        queryset = WaterBodyAvailability.objects.values('id','name')
        if request.method == 'GET':
            return Response(list(queryset))

class WaterBodyCatchmentTypeViewSet(ModelViewSet):
    http_method_names = ['get','post','patch','delete']
    queryset = WaterBodyCatchmentType.objects.all()
    serializer_class = WaterBodyCatchmentTypeSerializer
    filter_backends = (SearchFilter, OrderingFilter)
    pagination_class = LimitOffsetPagination
    search_fields = [ 'name' ]
    ordering_fields = [ 'name' ]
    ...
    @action(detail=False, methods=['GET'])
    def All(self,request):
        queryset = WaterBodyCatchmentType.objects.values('id','name')
        if request.method == 'GET':
            return Response(list(queryset))
...
class WaterBodyBundIssuesViewSet(ModelViewSet):
    http_method_names = ['get','post','patch','delete']
    queryset =  WaterBodyBundIssues.objects.all()
    serializer_class = WaterBodyBundIssuesSerializer
    filter_backends = (SearchFilter, OrderingFilter)
    pagination_class = LimitOffsetPagination
    search_fields = [ 'name' ]
    ordering_fields = [ 'name' ]
    ...
    @action(detail=False, methods=['GET'])
    def All(self,request):
        queryset =  WaterBodyBundIssues.objects.values('id','name')
        if request.method == 'GET':
            return Response(list(queryset))

class WaterBodySpreadIssuesViewSet(ModelViewSet):
    http_method_names = ['get','post','patch','delete']
    queryset =  WaterBodySpreadIssues.objects.all()
    serializer_class = WaterBodySpreadIssuesSerializer
    filter_backends = (SearchFilter, OrderingFilter)
    pagination_class = LimitOffsetPagination
    search_fields = [ 'name' ]
    ordering_fields = [ 'name' ]
    ...
    @action(detail=False, methods=['GET'])
    def All(self,request):
        queryset =  WaterBodySpreadIssues.objects.values('id','name')
        if request.method == 'GET':
            return Response(list(queryset))

class WaterBodyBarrelTypeViewSet(ModelViewSet):
    http_method_names = ['get','post','patch','delete']
    queryset =  WaterBodyBarrelType.objects.all()
    serializer_class = WaterBodyBarrelTypeSerializer
    filter_backends = (SearchFilter, OrderingFilter)
    pagination_class = LimitOffsetPagination
    search_fields = [ 'name' ]
    ordering_fields = [ 'name' ]
    ...
    @action(detail=False, methods=['GET'])
    def All(self,request):
        queryset =  WaterBodyBarrelType.objects.values('id','name')
        if request.method == 'GET':
            return Response(list(queryset))

class WaterBodyFieldChannelTypeViewSet(ModelViewSet):
    http_method_names = ['get','post','patch','delete']
    queryset =  WaterBodyFieldChannelType.objects.all()
    serializer_class = WaterBodyFieldChannelTypeSerializer
    filter_backends = (SearchFilter, OrderingFilter)
    pagination_class = LimitOffsetPagination
    search_fields = [ 'name' ]
    ordering_fields = [ 'name' ]
    ...
    @action(detail=False, methods=['GET'])
    def All(self,request):
        queryset =  WaterBodyFieldChannelType.objects.values('id','name')
        if request.method == 'GET':
            return Response(list(queryset))
...
class WaterBodyUpperTankSluiceResponseViewSet(ModelViewSet):
    http_method_names = ['get','post','patch','delete']
    serializer_class = WaterBodyUpperTankSluiceResponseSerializer

    def get_queryset(self):
        queryset = WaterBodyUpperTankSluiceResponse.objects.all()
        surveyResponse_id = self.request.query_params.get('surveyResponse_id')
        if surveyResponse_id is not None:
            queryset = queryset.filter(surveyResponse_id = surveyResponse_id)
...
class WaterBodySpringResponseViewSet(ModelViewSet):
    http_method_names = ['get','post','patch','delete']
    serializer_class = WaterBodySpringResponseSerializer

    def get_queryset(self):
        queryset = WaterBodySpringResponse.objects.all()
        surveyResponse_id = self.request.query_params.get('surveyResponse_id')
        if surveyResponse_id is not None:
            queryset = queryset.filter(surveyResponse_id = surveyResponse_id)
...
class WaterBodySubSurfaceResponseViewSet(ModelViewSet):
    http_method_names = ['get','post','patch','delete']
    serializer_class = WaterBodySubSurfaceResponseSerializer
...
class UserProfileViewSet(CreateModelMixin, RetrieveModelMixin, UpdateModelMixin, GenericViewSet):
    ...
    def updateUser(self, request, *args, **kwargs):
        ...
        with connection.cursor() as cursor:
          cursor.execute("update core_user set username=%s,first_name=%s,last_name=%s where id = %s", [username,first_name,last_name,user_id])
...
class TankImageViewSet(ModelViewSet):
    ...
    ordering_fields = [ 'filename','createdDate' ]
